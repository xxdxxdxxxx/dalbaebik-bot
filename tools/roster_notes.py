...
def install(a):
 ...
 def render(title,body):
  if request.path.startswith('/squads'):
   ...
   marker="</div><div id='sheet-status' class='sheet-status'>";body=body.replace(marker,"</div><div class='sheet-notes-grid'>"+''.join(notes)+"</div><div id='sheet-status' class='sheet-status'>",1)+JS
