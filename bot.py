...
# ЛС тем, кто ни разу не зашёл в войс (авто 19:50 + /ls_send)
...
# Отряд «Чемпионы» / замены — не пинговать ЛС перед КВ
SQUAD_SUBS_ID = 99
...
def default_db() -> dict[str, Any]:
    return {
        ...
        "absent_dm_sent": [],
        # id отряда (str) → название из базы
        "squad_names": {},
...
async def do_roster_remove(
    ...
    Удалить из состава:
      · по Discord (target)
      · или по нику STALCRAFT (game_nick) — как в админ-панели
    """
    data_preview = load_db()
    if not can_use_roster(actor, data_preview):
        allowed = get_access_role_ids(data_preview)
...
# ---------------------------------------------------------------------------
# История ников и сортировка состава
# ---------------------------------------------------------------------------
...
def reindex_scan_histories(data: dict[str, Any]) -> list[str]:
    ...
    Чтобы при смене регистра ника данные не «терялись».
    """
    changes: list[str] = []
...
def format_online_embed(data: dict[str, Any]) -> discord.Embed:
    ...
    if live:
        ...
        status = "⏳ День КВ · сессия откроется автоматически"
        color = COLOR_WAIT
    else:
...
def format_grenades_embed(data: dict[str, Any]) -> discord.Embed:
    ...
    # Всегда состав отрядов 1–6 + Чемпионы; «Без отряда» не показываем.
...
async def ensure_session_reset(
    ...
    *,
    ...
    force=True — всегда новый старт (тик старта явки / внутренний принудительный запуск).
    """
    n = now_msk()
    if not is_cw_day(n):
        return data
...
async def cmd_scan_now(
    ...
    if data.get("session_date") != today or not data.get("kv_session_active") or is_kv_finished(data):
        await interaction.response.send_message(
            "❌ Нет активной сегодняшней КВ-сессии. Дождитесь старта явки; сессия откроется автоматически по расписанию.",
            ephemeral=True,
        )
        return
